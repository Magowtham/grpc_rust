use todo::todo_server::TodoServer;
use tokio::sync::Mutex;

use todo::{todo_server::Todo, GetTodosResponse};
use todo::{CreateTodoRequest, CreateTodoResponse, TodoItem};
...
impl Todo for TodoService {
    async fn get_todos(&self, _: Request<()>) -> Result<Response<GetTodosResponse>, Status> {
        let message = GetTodosResponse {
            todos: self.todos.lock().await.to_vec(),
        };
    ...
    async fn create_todo(
        ...
        self.todos.lock().await.push(todo_item.clone());
